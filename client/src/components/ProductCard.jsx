      ...
      <div className="relative bg-white rounded-2xl shadow-2xl p-8 max-w-sm w-full text-center">
        ...
        <div className="flex gap-3">
          ...
          <button onClick={() => { onClose(); navigate("/auth"); }}
            className="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-bold py-2.5 rounded-xl text-sm transition shadow-md">
            Sign In
          </button>
        </div>
        ...
        <p className="text-xs text-gray-400 mt-4">
          No account?{" "}
          <button onClick={() => { onClose(); navigate("/auth?mode=register"); }}
...
export default function ProductCard({ product, compact = false }) {
  ...
  const handleWishlist = () => {
    if (!user) {
      setLoginPrompt("Please sign in to save items to your wishlist.");
      return;
    }
  ...
  const handleCheckout = () => {
    if (!user) {
      setLoginPrompt("Please sign in to proceed to checkout.");
      return;
    }
    addToCart(product);
    navigate("/checkout");
  ...
  // ── Compact variant (used in flash sale, featured rows) ───────────────────
  if (compact) {
    return (
      <>
          ...
          <div className="p-3">
            ...
            <div className="flex gap-1.5 mt-2">
              ...
              <button
                ...
                className="flex-1 bg-gray-900 hover:bg-gray-800 text-white text-xs font-semibold py-1.5 rounded-lg transition"
              >
                Checkout
              </button>
            </div>
      ...
      <div className="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 border border-gray-100 group flex flex-col">
        ...
        <div className="p-3 flex flex-col flex-1">
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={handleCheckout}
              className="flex-1 bg-gray-900 hover:bg-gray-800 text-white text-xs font-bold py-2 rounded-lg transition-colors"
            >
              Checkout
            </button>
          </div>
